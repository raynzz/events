...
async function createM2OField(collection, field, relatedCollection) {
    console.log(`Creating M2O field "${field}" in "${collection}" -> "${relatedCollection}"...`);
    await request(`/fields/${collection}`, 'POST', {
        field,
        type: 'integer', // ID is usually integer or uuid, assuming integer/auto-increment for now based on user request
        meta: {
    ...
    // We need to create the relationship separately or implicitly. 
    // Directus API usually handles relationship creation via /relations
...
async function setup() {
    console.log('🚀 Starting Full Directus Schema Setup...');
    ...
    try {
        ...
        // Relationships
        await createM2OField('events', 'owner', 'directus_users'); // Assuming directus_users is the system collection
        ...
        console.log('🎉 Full Schema Setup completed successfully!');
